...
#'
#' @export
collapsibleTreelocalOutput <- function(outputId, width = "100%", height = "400px") {
  shinyWidgetOutput(outputId, "collapsibleTreelocal", width, height, package = "collapsibleTreelocal")
}

#' @rdname collapsibleTree-shiny
#' @export
renderCollapsibleTreelocal <- function(expr, env = parent.frame(), quoted = FALSE) {
  if (!quoted) { expr <- substitute(expr) } # force quoted
  shinyRenderWidget(expr, collapsibleTreelocalOutput, env, quoted = TRUE)
}
